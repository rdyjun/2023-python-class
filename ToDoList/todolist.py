...
#----------------------------------------------------------
# 일정 리스트 박스 생성
tasks_listbox = tk.Listbox(window, width=50, height=20)
tasks_listbox.pack(padx=10, pady=10)
tasks_listbox.configure(bg='white', fg='#252525', highlightthickness=0, font=('Arial', 22))
#----------------------------------------------------------
# 입력하는 박스 생성
task_entry = tk.Entry(sub, width=50, height=3, highlightbackground='white', highlightthickness=0)
task_entry.pack(padx=10, pady=5)
task_entry.configure(bg='white', fg='#252525', font=('Arial', 20))
#----------------------------------------------------------
# 일정 생성 함수
def add_task():
    task = task_entry.get()
...
# 일정 추가 박스 생성
add_button = tk.Button(sub, text="추가하기", command=add_task, highlightbackground='white', width=100, height=3)
add_button.pack(side='bottom')
add_button.configure(fg=dk, bg=mc, font = ('Arial', 20, 'bold'))
#----------------------------------------------------------
# 일정 삭제 함수
def remove_task():
...
# 일정 삭제 박스 생성
remove_button = tk.Button(window, text="Remove Task", command=remove_task, highlightbackground='white', width=10, height=2)
...
# 전체 일정 삭제 버튼 생성
clear_button = tk.Button(window, text="모두 지우기", command=clear_tasks, highlightbackground='white', width=10, height=2)
